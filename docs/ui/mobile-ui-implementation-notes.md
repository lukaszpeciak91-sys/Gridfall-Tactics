...
- `FactionSelectScene` uses compact mobile faction cards with the header copy `Choose Your Faction` and the subtitle `Tap a faction to begin battle`. The header is intentionally smaller and more cinematic than the earlier prototype label.
...
- Card UI uses one card data source with multiple display modes: hand cards now use the HAND/FULL render mode with name, relevant unit stats, and `textShort`; full detail labels can include short rules text, while board/compact labels are not routed yet and must remain name plus ATK/HP/ARM-style stats only. Future board tap or long-press previews can expose full rules text without crowding the combat board.
