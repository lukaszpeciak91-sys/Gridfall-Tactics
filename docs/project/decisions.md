...
- Header titles are larger and placed lower than the original standard, creating a stronger TITLE → CONTENT relationship while remaining centered within a mobile-safe wrap width so Polish diacritics and English labels render without clipping in portrait layouts.
...
- Arena / Choose Faction, Collection, Tutorial, Settings, Rules, and future non-battle menu screens should reuse this same simplified header pattern to preserve the interdimensional prestige broadcast presentation system.
