...
- Feast is explicitly non-combat destruction: it destroys a friendly unit and damages the enemy base for 1, but it does not trigger Husk, Carrier, Rotcaller, Abomination, or Funeral Pyre.
...
| Swarm | Substrate | utility | - | enemy_all_armor_minus_1 | Enemies get -1 ARM this combat. | Deterministic enemy board debuff | Applies temporary -1 ARM to all current enemy units; no damage, draw, summon, or permanent HP/armor reduction. |
...
| Attrition Swarm | Feast | utility | - | destroy_friendly_damage_enemy_base_1 | Destroy an ally. Enemy base loses 1 HP. | Targeted friendly | Non-combat destruction plus enemy base damage; does not trigger combat-only death effects. |
