...
| Attrition Swarm | Leech | unit | 2/1/0 | leech_heal_hero_on_combat_kill | Combat kill and survive: heal your base 1. | Lane combat kill trigger | Heals its owner base by 1 after dealing lethal combat damage and surviving; base heal is capped by max HP. |
...
| Attrition Swarm | Funeral Pyre | order | - | funeral_pyre | First 2 [ALLIES] combat deaths:<br>each deal 1 ● to opposed enemy. | Non-targeted deterministic effect | Active for combat cleanup; cap 2 per owner per combat; damages only opposing enemy units in the dying allies’ lanes; no base fallback; multiple plays do not stack above cap. |
