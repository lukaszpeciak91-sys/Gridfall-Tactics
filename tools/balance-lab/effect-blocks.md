...
- `decay_hp_after_combat` — Unit passive for custom-faction testing. Units that were present at the start of a combat lane lose 1 HP after lane combat and combat-death cleanup. This is resolved as post-combat direct damage (not combat damage) through the normal non-combat damage/death cleanup pipeline, so death/Fallen handling is consistent without firing combat-death-only triggers.
