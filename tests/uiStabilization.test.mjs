...
test('StartScene uses optional logo art as the primary CTA with shared responsive layout', () => {
  ...
  assert.match(startSource, /this\.configureLogoActivation\(this\.title\)/);
  assert.match(startSource, /title\.setInteractive\(\{ useHandCursor: true \}\)/);
  assert.match(startSource, /title\.on\('pointerover', \(\) => this\.setLogoHoverState\(true\)\)/);
  assert.match(startSource, /title\.on\('pointerup', \(\) => this\.playStartTransition\(\)\)/);
...
test('start, main menu, and faction select use optional menu background art with dark fallback', () => {
  ...
  assert.match(startSource, /START_TRANSITION_MS = 360/);
  assert.match(startSource, /this\.scene\.start\('MainMenuScene', \{ revealFromStart: true \}\)/);
