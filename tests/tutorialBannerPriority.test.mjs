...
test('tutorial banner priority suppresses normal transient battle banners in tutorial mode', () => {
  ...
  assert.match(battleSource, /prepareTransientBattleBanner\(owner\) \{\s*const allowDuringTutorial = arguments\[1\]\?\.allowDuringTutorial === true;\s*if \(!allowDuringTutorial && this\.recoverTutorialBannerAfterSuppressedBattleBanner\?\.\(\)\) return false;/);
  ...
  assert.match(battleSource, /showInvalidActionBanner\(message\) \{[\s\S]*if \(!this\.prepareTransientBattleBanner\('invalid-action', \{ allowDuringTutorial \}\)\) \{\s*if \(!this\.shouldSuppressTransientBattleBannerForTutorial\(\)\) this\.deferTransientBattleBanner\('invalid-action'/);
