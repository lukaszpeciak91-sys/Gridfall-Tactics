...
test('mulligan tap toggles only mulligan selection and uses separate preview state', () => {
  assert.match(source, /if \(this\.openingMulliganPending\) \{\s*this\.selectedCardId = null;\s*this\.targetingState = null;\s*this\.pendingSwapIndex = null;\s*this\.previewedMulliganCardId = cardId;\s*this\.toggleOpeningMulliganCard\(cardId\);\s*return;\s*\}/);
...
test('gameplay zoom nudges to center while mulligan preview stays above its hand slot', () => {
  ...
  assert.match(source, /const MULLIGAN_HAND_CARD_PREVIEW_SCALE = 1\.16;/);
  assert.match(source, /const MULLIGAN_HAND_CARD_RAISE_RATIO = 0\.72;/);
