...
test('outside taps clear selection without intercepting board, pass, or card input', () => {
  ...
  assert.match(source, /clearOpeningMulliganPreviewFromOutsideTap\(pointer, currentlyOver = \[\]\) \{\s*const hasPreview = Boolean\(this\.previewedMulliganCardId \|\| this\.selectedHandCardZoom\);\s*if \(!hasPreview && this\.selectedMulliganCardIds\.length === 0\) return;[\s\S]*const preserveTutorialConfirmationSelection = \(this\.isTutorialBattle\?\.\(\) \?\? false\)[\s\S]*this\.getCurrentTutorialStep\?\.\(\)\?\.id === 'mulligan_confirm';\s*if \(!hasPreview && !preserveTutorialConfirmationSelection\) this\.selectedMulliganCardIds = \[\];\s*this\.updatePlayerBaseActionState\(\);\s*this\.resetCardHighlights\(\{ showPreview: false \}\);\s*\}/);
