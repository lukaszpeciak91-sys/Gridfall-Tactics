...
test('normal gameplay quick tap selects only and long press opens inspect while preserving active targeting', () => {
  ...
  assert.match(source, /startHandCardLongPress\(cardId\) \{\s*this\.cancelHandCardLongPress\(\);\s*this\.handCardLongPressEvent = this\.time\.delayedCall\(CARD_INSPECT_LONG_PRESS_MS, \(\) => \{[\s\S]*this\.longPressTriggeredCardId = cardId;[\s\S]*const preserveTargetingSession = this\.selectedCardId === cardId && Boolean\(this\.targetingState\);\s*if \(!preserveTargetingSession\) \{\s*this\.selectedCardId = null;\s*this\.targetingState = null;\s*this\.effectCastState = null;\s*this\.actionMode = null;\s*this\.destroyTargetingInstruction\(\);\s*\}\s*this\.hoverInspectCardId = cardId;[\s\S]*this\.resetCardHighlights\(\{ showPreview: true \}\);/);
