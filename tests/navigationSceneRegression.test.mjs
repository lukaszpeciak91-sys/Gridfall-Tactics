...
test('battle utility menu opens panel actions and rules resume the existing scene', () => {
  ...
  assert.match(battleSource, /calculateBattleUtilityMenuLayout\(\{ width, height, margin, triggerX, triggerY, triggerWidth, triggerHeight, actionCount: utilityMenuActions\.length \}\)/);
  assert.match(battleSource, /panelLeft,[\s\S]*menuScale,[\s\S]*basePanelContentWidth,[\s\S]*panelHorizontalPadding,[\s\S]*panelWidth,[\s\S]*panelHeight,[\s\S]*panelTop,[\s\S]*panelX,[\s\S]*panelY,[\s\S]*rowY,/);
  assert.doesNotMatch(battleSource, /utilityMenuTitle|TACTICAL MENU/);
  assert.match(battleSource, /const muteToggle = createMuteToggleControl\(this, panelX - 28, rowY, 42, \{ depth: depth \+ 3, ambientFrameSweep: false, ambientRingMotion: false \}\);[\s\S]*const fullscreenToggle = createFloatingControl\(this, panelX \+ 28, rowY, 42, NAVIGATION_ICON_TYPES\.FULLSCREEN/);
  assert.match(battleSource, /showUtilityMenuPanel\(\) \{[\s\S]*this\.closeInspectPreview\(\{ animate: false, clearSelection: true \}\);[\s\S]*labelKey: 'ui\.battle\.utilityMenuRules'[\s\S]*labelKey: 'ui\.battle\.utilityMenuSettings'[\s\S]*labelKey: 'ui\.battle\.utilityMenuSurrender'[\s\S]*outsideCatcher\.on\('pointerup',[\s\S]*this\.guardPointerEvent\(pointer\);[\s\S]*this\.destroyUtilityMenuPanel\(\);[\s\S]*createMuteToggleControl\(this,[\s\S]*utilityMenuActions\.map\(\(action, index\) => this\.createUtilityMenuButton/);
...
test('rules panel contains glossary-first player-facing rules summary', () => {
  ...
  assert.match(source, /ATK — Combat damage dealt in combat/);
  assert.match(source, /ARM — Armor that reduces combat damage/);
  ...
  assert.match(source, /Both Bases begin with 12 HP/);
  assert.match(source, /Alternating Initiative/);
  assert.match(source, /Each side takes one action/);
  assert.match(source, /all lanes resolve combat/);
  assert.match(source, /the side that acted second goes first/);
  assert.match(source, /Before battle, you may replace up to 2 cards/);
  assert.match(source, /There are three combat lanes/);
  assert.match(source, /Effect cards use your action and may require a target/);
  assert.doesNotMatch(source, /hold PASS to surrender/i);
  assert.doesNotMatch(source, /PASS and Surrender/);
  assert.match(source, /Can happen only when at least one Base has 3 HP or less/);
  assert.match(source, /Tap one of your units/);
  assert.match(source, /PASS uses your action/);
