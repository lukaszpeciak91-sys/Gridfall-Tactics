...
test('battle utility menu opens panel actions and rules resume the existing scene', () => {
  ...
  assert.match(battleSource, /const panelLeft = triggerX \+ triggerWidth \/ 2;[\s\S]*const menuScale = 1\.1;[\s\S]*const basePanelContentWidth = 208;[\s\S]*const panelContentWidth = Math\.round\(basePanelContentWidth \* menuScale\);[\s\S]*const panelHorizontalPadding = Math\.round\(4 \* menuScale\);[\s\S]*const panelWidth = Math\.min\(panelContentWidth \+ panelHorizontalPadding \* 2, width - margin - panelLeft\);[\s\S]*const panelHeight = Math\.round\(192 \* menuScale\);[\s\S]*const panelTop = triggerY - triggerHeight \/ 2 - \(panelHeight - 228\) \/ 2;[\s\S]*const panelX = Math\.min\(width - margin - panelWidth \/ 2, panelLeft \+ basePanelContentWidth \/ 2 \+ 14\);[\s\S]*const panelY = panelTop \+ panelHeight \/ 2;[\s\S]*const rowY = panelTop \+ Math\.round\(28 \* menuScale\);/);
  ...
  assert.match(battleSource, /showUtilityMenuPanel\(\) \{[\s\S]*this\.closeInspectPreview\(\{ animate: false, clearSelection: true \}\);[\s\S]*outsideCatcher\.on\('pointerup',[\s\S]*this\.guardPointerEvent\(pointer\);[\s\S]*this\.destroyUtilityMenuPanel\(\);[\s\S]*createMuteToggleControl\(this,[\s\S]*translateActive\('ui\.battle\.utilityMenuRules', 'Rules'\), \(\) => this\.openRulesPanel\(\)\),[\s\S]*translateActive\('ui\.battle\.utilityMenuSettings', 'Settings'\), \(\) => this\.openSettingsScene\(\)\),[\s\S]*translateActive\('ui\.battle\.utilityMenuReturn', 'Return'\), \(\) => this\.handleUtilityMenuReturn\(\)\),[\s\S]*translateActive\('ui\.battle\.utilityMenuMainMenu', 'Main Menu'\), \(\) => this\.handleUtilityMenuMainMenu\(\)\),[\s\S]*\}/);
