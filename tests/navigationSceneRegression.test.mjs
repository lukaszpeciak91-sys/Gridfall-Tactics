...
test('battle utility menu opens panel actions and rules resume the existing scene', () => {
  ...
  assert.match(battleSource, /const panelLeft = triggerX \+ triggerWidth \/ 2;[\s\S]*const panelContentWidth = 208;[\s\S]*const panelHorizontalPadding = 4;[\s\S]*const panelWidth = Math\.min\(panelContentWidth \+ panelHorizontalPadding \* 2, width - margin - panelLeft\);[\s\S]*const panelHeight = 228;[\s\S]*const panelTop = triggerY - triggerHeight \/ 2;[\s\S]*const panelX = panelLeft \+ panelContentWidth \/ 2 \+ 14;[\s\S]*const panelY = panelTop \+ panelHeight \/ 2;[\s\S]*const rowY = panelTop \+ 28;/);
