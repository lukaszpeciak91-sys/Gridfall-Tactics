...
test('BattleScene routes targeted immediate combat events through normal combat animation before refresh', () => {
  ...
  assert.match(source, /this\.refreshBoardLabelsFromSnapshot\(combatSnapshot\.board, combatSnapshot\.offlineReservations, combatSnapshot\);[\s\S]*?if \(onDeployBeamEvent\)[\s\S]*?await this\.animateBeamAttack\([\s\S]*?else \{\s*await this\.playCombatAnimations\(combatEvents, combatSnapshot\.board\);/);
...
test('BattleScene plays first-class death-trigger events inline and dedupes legacy reconstruction', () => {
  ...
  assert.match(source, /await this\.playDeathTriggerPresentationEvent\(event, preCombatBoardSnapshot\);[\s\S]*?animationCompletionTimestamp[\s\S]*?continue;/);
