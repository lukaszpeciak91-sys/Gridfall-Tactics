...
test('polished card text stays within mobile collection and inspect rules panels', () => {
  const cardCases = [
    ...
    { factionKey: "Control", cardId: "control_sniper_1", locale: "en", expectedBody: "On deploy choose any ♟ and deal damage" },
    { factionKey: "Control", cardId: "control_sniper_1", locale: "pl", expectedBody: "Po wystawieniu wybierz dowolnego ♟ i zadaj obrażenia" },
...
test('inline effect icon typography uses glyph-sized symbols, centered baseline, and compact icon spacing', () => {
  ...
  assert.equal(INLINE_GAMEPLAY_ICON_TRAILING_SPACE_SCALE, 1.15);
...
test('inline gameplay icons keep natural leading spacing and normalized trailing spacing while stat icons retain compact numeric spacing', () => {
  ...
  assert.equal(allyLine.segments[2].x, 102);
...
test('inline icon layout applies word spacing and compact stat-number spacing across edge cases', () => {
  ...
  const gameplayTrailingCases = [
    ['Atakuje ♙ text', '♙', 'text'],
    ['Atakuje ♙♙ text', '♙♙', 'text'],
    ['Atakuje ♟ text', '♟', 'text'],
    ['Atakuje ♟♟ text', '♟♟', 'text'],
  ];
  for (const [text, icon, next] of gameplayTrailingCases) {
    assert.equal(gapBetween(line(text).at(0).segments, icon, next), 12, `${icon} trailing spacing uses 1.15 scale`);
  }
  ...
  assert.equal(gapBetween(consecutive, '♙', 'B'), 12);
  assert.equal(gapBetween(consecutive, 'B', '♟'), 10);
  assert.equal(gapBetween(consecutive, '♟', 'C'), 12);
});
...
test('gameplay icon trailing spacing is normalized for validation card patterns without source-string padding', () => {
  ...
  for (const { factionKey, cardId, locale, icon, next } of validationCases) {
    ...
    assert.equal(nextSegment.x - (segments[iconIndex].x + segments[iconIndex].width), 12, `${cardId} normalized icon trailing gap`);
...
test('Wezwanie Grobu Polish source copy has no colon after the ally icon marker', () => {
  const pl = JSON.parse(readFileSync(new URL('../src/localization/translations/pl.json', import.meta.url), 'utf8'));
  const graveCallText = pl.cards.attrition_swarm_grave_call_1.textShort;
  assert.equal(graveCallText, 'Przywołaj 1/1\nBrak [ALLY] przywołaj do 2');
  assert.doesNotMatch(graveCallText, /\[ALLY\]:/u);
});
